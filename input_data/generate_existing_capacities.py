...
def generate_existing_capacities(file_demand, lifetime, technology, max_load):
    ...
    if lifetime % 2 == 0:
        capacity_existing['capacity_existing'] = capacity_existing['demand'] / max_load / (lifetime / 2 - 1)

        for index, row in capacity_existing.iterrows():
            for year in range(0, lifetime - 2, 2):
                new_row = row.copy()
                new_row['year_construction'] = 2022 - year
                new_rows.append(new_row)

    else:
        capacity_existing['capacity_existing'] = capacity_existing['demand'] / max_load / ((lifetime/2) - 0.5)
        for index, row in capacity_existing.iterrows():
            for year in range(0, lifetime - 2, 2):
                new_row = row.copy()
...
def generate_existing_capacity_ASU(file_demand, lifetime, technology, max_load):
    ...
    if lifetime % 2 == 0:
        capacity_existing['capacity_existing'] = capacity_existing['demand'] * 0.823 / max_load / (lifetime / 2 - 1)
        ...
        capacity_existing['capacity_existing'] = capacity_existing['demand'] * 0.823 / max_load / ((lifetime / 2) - 0.5)
        for index, row in capacity_existing.iterrows():
            for year in range(0, lifetime - 2, 2):
                new_row = row.copy()
                new_row['year_construction'] = 2023 - year
...
def generate_existing_capacities_steel(file_demand, lifetime_BF_BOF, lifetime_EAF, lifetime_DRI, max_load_BF_BOF, max_load_EAF, max_load_DRI):
    ...
    capacity_existing_BF_BOF['capacity_existing'] = capacity_existing_BF_BOF['demand'] * 0.7 / max_load_BF_BOF / (lifetime_BF_BOF / 2 - 1)

    new_rows = []

    for index, row in capacity_existing_BF_BOF.iterrows():
        for year in range(0, lifetime_BF_BOF - 2, 2):
            new_row = row.copy()
            new_row['year_construction'] = 2022 - year
            new_rows.append(new_row)
    ...
    capacity_existing_EAF['capacity_existing'] = capacity_existing_EAF['demand'] * 0.3 / max_load_EAF / (lifetime_EAF/2 - 0.5)

    new_rows = []

    for index, row in capacity_existing_EAF.iterrows():
        for year in range(0, lifetime_EAF - 2, 2):
            new_row = row.copy()
            new_row['year_construction'] = 2023 - year
            new_rows.append(new_row)
    ...
    for index, row in capacity_existing_DRI.iterrows():
        for year in range(0, lifetime_DRI - 2, 2):
            new_row = row.copy()
            new_row['year_construction'] = 2023 - year
            new_rows.append(new_row)
