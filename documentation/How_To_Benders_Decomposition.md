...
- The parameters `place_upper_bound_capacity_expansion` is essential when the user want to maximize the objective function in order to not encounter an unbounded master problem.
- There is also the possibility to use the `cross over method`. If the problem allows, this will check the optimal solutions of all the subproblems to cross verify if these are feasible for the overall configuration. If this is the case, the solution is already found. Otherwise, the algorithm generate feasibility cuts to add to the master. Since this cuts are generate from solutions that are optimal at least for one subproblem, they are really informative and can guide better the convergence, reducing the total amount of time needed. There is of course a trade off between the `cross over method` and the memory-time burden. **TO NOTE:** This method works only if Benders Decomposition is used as feasibility checks. If also optimality cuts need to be added to the master, the method still need to be integrated and tested.
...
**TO NOTE:** It is recommended to not use the scaling algorithm when running Benders Decompsoition. Since we are dealing with multiple uncertainty parameters all the subproblems will be scaled in a different way, leading to non converging cuts. 
