...
if __name__ == "__main__":
    ...
    if flag_endog:
        ...
        var_segment_position = r.get_df("total_cost_pwa_cum_capacity_segment_position")
        var_segment_selection = r.get_df("total_cost_pwa_segment_selection")
        var_total_global_cost = r.get_df("total_cost_pwa_global_cost")
        var_capacity = r.get_df("capacity")
        var_capacity_addition = r.get_df("capacity_addition")
        var_cost_capex = r.get_df("cost_capex")
        var_global_capacity = r.get_df("global_cumulative_capacity")
        var_demand = r.get_df("demand")

        # Read Parameters
        par_slope = r.get_df("total_cost_pwa_slope")
        par_intersect = r.get_df("total_cost_pwa_intersect")
        par_global_share = r.get_df("global_share_factor")
        par_total_cost_pwa_points_lower_bound = r.get_df("total_cost_pwa_points_lower_bound")
        par_total_cost_pwa_points_upper_bound = r.get_df("total_cost_pwa_points_upper_bound")
        par_total_cost_pwa_TC_upper_bound = r.get_df("total_cost_pwa_TC_upper_bound")
        par_total_cost_pwa_TC_lower_bound = r.get_df("total_cost_pwa_TC_lower_bound")
        par_total_cost_pwa_initial_global_cost = r.get_df("total_cost_pwa_initial_global_cost")
        par_capacity_existing = r.get_df("capacity_existing")
        ...
        diff_total_cost = (var_total_global_cost - calc_total_cost).sum()
        print(f"The difference between the variable total cost and the calculated total cost is {diff_total_cost}.")

        # Plot the total cost points on the curves
        ...
        for tech in var_capacity.index.get_level_values("technology").unique():
            for capacity_type in var_capacity.loc[tech].index.get_level_values("capacity_type").unique():
                ...
                interpolated_q = r.get_df("total_cost_pwa_points_lower_bound").loc[tech, capacity_type, :].values
                interpolated_q = np.append(interpolated_q, r.get_df("total_cost_pwa_points_upper_bound").loc[tech, capacity_type, :].values[-1])

                interpolated_TC = r.get_df("total_cost_pwa_TC_lower_bound").loc[tech, capacity_type, :].values
                interpolated_TC = np.append(interpolated_TC, r.get_df("total_cost_pwa_TC_upper_bound").loc[tech, capacity_type, :].values[-1])
