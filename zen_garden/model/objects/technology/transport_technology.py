...
class TransportTechnology(Technology):
    ...
    def get_transport_loss_factor(self):
        ...
        self.transport_loss_factor_linear = self.data_input.extract_input_data(
        ...
        if self.name in self.optimization_setup.system["set_transport_technologies_loss_exponential"]:
            assert (
                "transport_loss_factor_exponential" in self.data_input.attribute_dict
            ), f"The transport technology {self.name} has no transport_loss_factor_exponential attribute."
            self.transport_loss_factor_exponential = self.data_input.extract_input_data(
    ...
    def get_capex_transport(self):
        ...
        if self.optimization_setup.system["double_capex_transport"]:
            ...
            if "capex_per_distance_transport" in self.data_input.attribute_dict:
                ...
                raise AttributeError(
                    f"The transport technology {self.name} has neither capex_per_distance_transport nor capex_specific_transport attribute."
                )
        ...
        if "opex_specific_fixed_per_distance" in self.data_input.attribute_dict:
            ...
            raise AttributeError(
                f"The transport technology {self.name} has neither opex_specific_fixed_per_distance nor opex_specific_fixed attribute."
            )
    ...
    def construct_params(cls, optimization_setup):
        ...
        optimization_setup.parameters.add_parameter(
            name="transport_loss_factor_exponential",
            index_names=["set_transport_technologies"],
    ...
    def construct_vars(cls, optimization_setup):
        ...
        def flow_transport_bounds(index_values, index_list):
            ...
            time_step_year = xr.DataArray(
                [
                    optimization_setup.energy_system.time_steps.convert_time_step_operation2year(time)
    ...
    def disjunct_on_technology(cls, optimization_setup, tech, capacity_type, edge, time, binary_var):
        ...
        lhs = (
            model.variables["flow_transport"].loc[tech, edge, time]
            - params.min_load.loc[tech, capacity_type, edge, time]
            * model.variables["capacity"].loc[tech, capacity_type, edge, time_step_year]
        )
        rhs = 0
        constraint = lhs >= rhs
        ...
        constraints.add_constraint_block(
            model,
            name=f"disjunct_transport_technology_min_load_{tech}_{capacity_type}_{edge}_{time}",
    ...
    def disjunct_off_technology(cls, optimization_setup, tech, capacity_type, edge, time, binary_var):
        ...
        constraints.add_constraint_block(
            model,
            name=f"disjunct_transport_technology_off_{tech}_{capacity_type}_{edge}_{time}_lower",
            constraint=(model.variables["flow_transport"][tech, edge, time].to_linexpr() == 0),
            disjunction_var=binary_var,
        )
...
class TransportTechnologyRules(GenericRule):
    ...
    def constraint_transport_technology_losses_flow(self):
        ...
        exp_techs = pd.Series(
            {
                t: True if t in self.system["set_transport_technologies_loss_exponential"] else False
                for t in self.sets["set_transport_technologies"]
            }
        )
        ...
        if len(exp_techs) == 0:
            return None
        exp_loss_factor = (
            np.exp(-self.parameters.transport_loss_factor_exponential * self.parameters.distance)
        ).broadcast_like(flow_transport.lower)
        lin_loss_factor = (self.parameters.transport_loss_factor_linear * self.parameters.distance).broadcast_like(
    ...
    def constraint_transport_technology_capex(self):
        ...
        index_values, index_list = Element.create_custom_set(
            ["set_transport_technologies", "set_edges", "set_time_steps_yearly"], self.optimization_setup
        )
        ...
        term_distance_not_inf = (1 - mask) * (
            self.variables["cost_capex"].loc[coords[0], "power", coords[1], coords[2]]
        ...
        if np.any(
            ...
            * self.parameters.capex_per_distance_transport.loc[coords[0], coords[1]]
            ...
            term_distance_not_inf -= (
                (1 - mask)
                * self.variables["technology_installation"].loc[coords[0], "power", coords[1], coords[2]]
                * (
                    self.parameters.distance.loc[coords[0], coords[1]]
                    * self.parameters.capex_per_distance_transport.loc[coords[0], coords[1]]
                )
