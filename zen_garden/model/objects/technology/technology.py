...
class Technology(Element):
    ...
    def store_input_data(self):
        ...
        self.capacity_addition_min = self.data_input.extract_input_data(
            "capacity_addition_min", index_sets=[], unit_category={"energy_quantity": 1, "time": -1}
        )
        self.capacity_addition_max = self.data_input.extract_input_data(
            "capacity_addition_max", index_sets=[], unit_category={"energy_quantity": 1, "time": -1}
        )
        self.capacity_addition_unbounded = self.data_input.extract_input_data(
            "capacity_addition_unbounded", index_sets=[], unit_category={"energy_quantity": 1, "time": -1}
        )
        ...
        self.raw_time_series["opex_specific_variable"] = self.data_input.extract_input_data(
        ...
        if self.__class__.__name__ == "StorageTechnology":
            ...
            capex_capacity_existing = capacities_existing.to_frame().apply(
                lambda _capacity_existing: self.calculate_capex_of_single_capacity(
                    _capacity_existing.squeeze(), _capacity_existing.name, storage_energy
                ),
                axis=1,
            )
        else:
    ...
    def get_available_existing_quantity(
        ...
        for id_capacity_existing in sets["set_technologies_existing"][tech]:
            is_existing = cls.get_if_capacity_still_existing(
                optimization_setup, tech, year, loc=loc, id_capacity_existing=id_capacity_existing
            )
    ...
    def construct_sets(cls, optimization_setup):
        ...
        optimization_setup.sets.add_set(
            ...
            data=optimization_setup.get_attribute_of_all_elements(cls, "set_technologies_existing"),
    ...
    def construct_params(cls, optimization_setup):
        ...
        optimization_setup.parameters.add_parameter(
            ...
            index_names=["set_technologies", "set_capacity_types", "set_location", "set_technologies_existing"],
            capacity_types=True,
        ...
        optimization_setup.parameters.add_parameter(
            ...
            index_names=["set_technologies", "set_location", "set_technologies_existing"],
        ...
        optimization_setup.parameters.add_parameter(
            name="min_load",
            index_names=["set_technologies", "set_capacity_types", "set_location", "set_time_steps_operation"],
            capacity_types=True,
        ...
        optimization_setup.parameters.add_parameter(
            ...
            data=cls.get_existing_quantity(optimization_setup, type_existing_quantity="capacity"),
            doc="Parameter which specifies the total available capacity of existing technologies at the beginning of the optimization",
            calling_class=cls,
        )
        optimization_setup.parameters.add_parameter(
            name="existing_capex",
            data=cls.get_existing_quantity(optimization_setup, type_existing_quantity="cost_capex"),
    ...
    def construct_vars(cls, optimization_setup):
        ...
        def capacity_bounds(tech, capacity_type, loc, time):
            ...
            if tech in techs_on_off:
                ...
                bound_capacity = min(
                    capacity_addition_max + capacities_existing, max_capacity_limit + capacities_existing
                )
    ...
    def construct_constraints(cls, optimization_setup):
        ...
        constraints.add_constraint_block(
            model,
            name="constraint_technology_min_capacity_addition",
        ...
        constraints.add_constraint_block(
            model,
            name="constraint_technology_max_capacity_addition",
        ...
        constraints.add_constraint_block(
            model,
            name="constraint_technology_diffusion_limit_total",
        ...
        constraints.add_constraint_rule(
            model,
            name="constraint_cost_capex_total",
            index_sets=sets["set_time_steps_yearly"],
        ...
        constraints.add_constraint_rule(
            model,
            name="constraint_cost_opex_total",
            index_sets=sets["set_time_steps_yearly"],
        ...
        constraints.add_constraint_block(
            model,
            name="constraint_carbon_emissions_technology_total",
        ...
        model.add_constraints(
            model.variables["tech_on_var"] + model.variables["tech_off_var"] == 1, name="tech_on_off_cons"
        )
    ...
    def get_existing_quantity(cls, optimization_setup, type_existing_quantity):
        ...
        for i, (tech, capacity_type, loc, time) in enumerate(index_values):
            values[i] = Technology.get_available_existing_quantity(
                optimization_setup, tech, capacity_type, loc, time, type_existing_quantity=type_existing_quantity
            )
...
class TechnologyRules(GenericRule):
    ...
    def disjunct_on_technology_rule(self, tech, capacity_type, loc, time):
        ...
        for subclass in Technology.__subclasses__():
            if tech in self.optimization_setup.get_all_names_of_elements(subclass):
                ...
                subclass.disjunct_on_technology_rule(
                    self.optimization_setup, tech, capacity_type, loc, time, binary_var
                )
                return None
    ...
    def disjunct_off_technology_rule(self, tech, capacity_type, loc, time):
        ...
        for subclass in Technology.__subclasses__():
            if tech in self.optimization_setup.get_all_names_of_elements(subclass):
                ...
                subclass.disjunct_off_technology_rule(
                    self.optimization_setup, tech, capacity_type, loc, time, binary_var
                )
                return None
    ...
    def constraint_cost_capex_total_rule(self, year):
        ...
        lhs = self.variables["cost_capex_total"].loc[year] - term_sum_yearly
        rhs = 0
        constraints = lhs == rhs
    ...
    def constraint_cost_opex_total_rule(self, year):
        ...
        ### formulate constraint
        lhs = self.variables["cost_opex_total"].loc[year] - term_sum_yearly
        rhs = 0
        constraints = lhs == rhs
    ...
    def constraint_technology_min_capacity_addition_block(self):
        ...
        ### index sets
        index_values, index_names = Element.create_custom_set(
            ["set_technologies", "set_capacity_types", "set_location", "set_time_steps_yearly"], self.optimization_setup
        )
        index = ZenIndex(index_values, index_names)
        ...
        ### formulate constraint
        lhs = mask * (
            self.parameters.capacity_addition_min * self.variables["technology_installation"]
    ...
    def constraint_technology_max_capacity_addition_block(self):
        ...
        for tech, capacity_type in index.get_unique(["set_technologies", "set_capacity_types"]):
            ...
            if self.parameters.capacity_addition_max.loc[tech, capacity_type] != np.inf:
                ...
                if np.any(
                    self.parameters.capacity_addition_max.loc[tech, capacity_type].notnull()
                    & (self.parameters.capacity_addition_max.loc[tech, capacity_type] != 0)
                ):
                    term_installation = (
                        self.parameters.capacity_addition_max.loc[tech, capacity_type].item()
                ...
                ### formulate constraint
                lhs = -self.variables["capacity_addition"].loc[tech, capacity_type] + term_installation
                rhs = 0
        ...
        for tech, year in index.get_unique(["set_technologies", "set_time_steps_yearly"]):
            ...
            for previous_year in Technology.get_lifetime_range(self.optimization_setup, tech, year):
                term_neg_previous_capacity_additions.append(
                    -1.0 * self.variables["capacity_addition"].loc[tech, :, :, previous_year]
                )

            ### formulate constraint
            lhs = lp_sum(
                [1.0 * self.variables["capacity"].loc[tech, :, :, year], *term_neg_previous_capacity_additions]
            )
    ...
    def constraint_technology_diffusion_limit_block(self):
        ...
        for tech, time in index.get_unique(["set_technologies", "set_time_steps_yearly"]):
            ...
            if self.parameters.max_diffusion_rate.loc[tech, time] != np.inf:
                ...
                capacity_types = index.get_values(
                    [tech, slice(None), slice(None), time], "set_capacity_types", unique=True
                )
                mask = xr.DataArray(
                    np.nan, coords=[self.variables.coords["set_capacity_types"]], dims=["set_capacity_types"]
                )
                ...
                term_total_capacity_knowledge_existing = (
                    (
                        self.parameters.capacity_existing.loc[
                            tech, :, set_locations, existing_time
                        ]  # add spillover from other regions
                        + knowledge_spillover_rate
                        * (
                            self.parameters.capacity_existing.loc[tech, :, set_locations, existing_time].sum(
                                "set_location"
                            )
                    ...
                    ** (
                        delta_time
                        + self.parameters.lifetime.loc[tech].item()
                        - self.parameters.lifetime_existing.loc[tech, set_locations, existing_time]
                    )
                ...
                horizon_time = self.variables.coords["set_time_steps_yearly"][
                    self.variables.coords["set_time_steps_yearly"].isin(horizon_time)
                ]
                if len(horizon_time) >= 1:
                    term_total_capacity_knowledge_addition = (
                        (
                            self.variables["capacity_addition"].loc[
                                tech, :, set_locations, horizon_time
                            ]  # add spillover from other regions
                            + knowledge_spillover_rate
                            * (
                                self.variables["capacity_addition"]
                                .loc[tech, :, set_locations, horizon_time]
                                .sum("set_location")
                    ...
                    # dummy term
                    term_total_capacity_knowledge_addition = (
                        self.variables["capacity_investment"].loc[tech, :, set_locations, time].where(False)
                    )

                # total capacity in previous year; if time is first time step of interval, use existing capacities of present year
                other_techs = [
                    other_tech
                    for other_tech in set_technology
                    if self.sets["set_reference_carriers"][other_tech][0] == reference_carrier
                ]
                if time != self.optimization_setup.energy_system.set_time_steps_yearly[0]:
                    term_total_capacity_all_techs_var = (
                        self.variables["capacity"].loc[other_techs, :, set_locations, time - 1].sum("set_technologies")
                    )
                    term_total_capacity_all_techs_param = xr.zeros_like(
                        self.parameters.existing_capacities.loc[tech, :, set_locations, time]
                    )
                else:
                    term_total_capacity_all_techs_param = self.parameters.existing_capacities.loc[
                        other_techs, :, set_locations, time
                    ].sum("set_technologies")
                    term_total_capacity_all_techs_var = (
                        self.variables["capacity"].loc[tech, :, set_locations, time].where(False)
                    )

                ### formulate constraint
                # build the lhs
                lhs = (
                    self.variables["capacity_investment"].loc[tech, :, set_locations, time]
                    + self.variables["capacity_investment"].loc[tech, :, :, time].where(False)
                    - ((1 + self.parameters.max_diffusion_rate.loc[tech, time].item()) ** interval_between_years - 1)
                    * term_total_capacity_knowledge_addition
    ...
    def constraint_technology_diffusion_limit_total_block(self):
        ...
        for tech, time in index.get_unique(["set_technologies", "set_time_steps_yearly"]):
            ...
            if self.parameters.max_diffusion_rate.loc[tech, time] != np.inf:
                ...
                ).sum(["set_technologies_existing", "set_location"])
                # round to avoid numerical errors
                # if self.optimization_setup.solver["round_parameters"]:
                #     rounding_value = 10 ** (-self.optimization_setup.solver["rounding_decimal_points_capacity"])
                #     term_total_capacity_knowledge_existing = term_total_capacity_knowledge_existing.where(term_total_capacity_knowledge_existing > rounding_value, 0)
                ...
                if len(horizon_time) >= 1:
                    term_total_capacity_knowledge_addition = (
                        self.variables["capacity_addition"].loc[tech, :, set_locations, horizon_time]
                        * (1 - knowledge_depreciation_rate) ** (interval_between_years * (end_time - horizon_time))
                    ).sum(["set_time_steps_yearly", "set_location"])
                else:
                    # dummy term
                    term_total_capacity_knowledge_addition = (
                        self.variables["capacity_investment"]
                        .loc[tech, :, set_locations, time]
                        .where(False)
                        .sum("set_location")
                    )
                ...
                if time != self.optimization_setup.energy_system.set_time_steps_yearly[0]:
                    term_total_capacity_all_techs_var = (
                        ...
                        .loc[other_techs, :, set_locations, time - 1]
                    ...
                    term_total_capacity_all_techs_var = (
                        self.variables["capacity"].loc[tech, :, set_locations, time].where(False).sum("set_location")
                    )

                ### formulate constraint
                # build the lhs
                lhs = (
                    self.variables["capacity_investment"].loc[tech, :, set_locations, time].sum("set_location")
                    - ((1 + self.parameters.max_diffusion_rate.loc[tech, time].item()) ** interval_between_years - 1)
                    * term_total_capacity_knowledge_addition
                    - self.parameters.market_share_unbounded * term_total_capacity_all_techs_var
                )
        ...
        return self.constraints.return_contraints(
            constraints, model=self.model, stack_dim_name="diffusion_limit_total_dim"
        )
    ...
    def constraint_capex_yearly_block(self):
        ...
        for tech, year in index.get_unique(["set_technologies", "set_time_steps_yearly"]):
            ...
            ### formulate constraint
            lhs = lp_sum(
                [1.0 * self.variables["capex_yearly"].loc[tech, :, :, year], *term_neg_annuity_cost_capex_previous]
            )
        ...
        for tech in index.get_unique(["set_technologies"]):
            ...
            if tech in self.sets["set_conversion_technologies"]:
                if reference_carrier in self.sets["set_input_carriers"][tech]:
                    reference_flow = (
                        self.variables["flow_conversion_input"].loc[tech, reference_carrier, locs].to_linexpr()
                    )
                    reference_flow = reference_flow.rename({"set_nodes": "set_location"})
                else:
                    reference_flow = (
                        self.variables["flow_conversion_output"].loc[tech, reference_carrier, locs].to_linexpr()
                    )
                    reference_flow = reference_flow.rename({"set_nodes": "set_location"})
            ...
            lhs = lp.merge(
                self.variables["cost_opex"].loc[tech].where(False).to_linexpr(),
    ...
    def constraint_opex_yearly_block(self):
        ...
        for tech, year in index.get_unique(["set_technologies", "set_time_steps_yearly"]):
            ...
            term_neg_summed_cost_opex = -(
                self.variables["cost_opex"].loc[tech, :, times]
                * self.parameters.time_steps_operation_duration.loc[times]
            ).sum(["set_time_steps_operation"])
            term_neg_summed_capacities = -lp_sum(
                [
                    self.parameters.opex_specific_fixed.loc[tech, capacity_type, :, year]
                    * self.variables["capacity"].loc[tech, capacity_type, :, year]
                    for capacity_type in self.system["set_capacity_types"]
                    if tech in self.sets["set_storage_technologies"]
                    or capacity_type == self.system["set_capacity_types"][0]
                ]
            )
            ...
            lhs = lp_sum(
                [
                    1.0 * self.variables["opex_yearly"].loc[tech, :, year],
                    term_neg_summed_cost_opex,
    ...
    def constraint_carbon_emissions_technology_block(self):
        ...
        for tech in index.get_unique(["set_technologies"]):
            ...
            # the first term is just to ensure full shape
            lhs = lp.merge(
                self.variables["carbon_emissions_technology"].loc[tech].where(False).to_linexpr(),
    ...
    def constraint_carbon_emissions_technology_total_block(self):
        ...
        index_values, index_names = Element.create_custom_set(
            ["set_technologies", "set_location"], self.optimization_setup
        )
        index = ZenIndex(index_values, index_names)
        ...
        for year in years:
            ...
            for tech in index.get_unique(["set_technologies"]):
                locs = index.get_values([tech], "set_location", unique=True)
                times = self.time_steps.get_time_steps_year2operation(year)
                term_summed_carbon_emissions_technology.append(
                    (
                        self.variables["carbon_emissions_technology"].loc[tech, locs, times]
                        * self.parameters.time_steps_operation_duration.loc[times]
                    ).sum()
                )
            ...
            ### formulate constraint
            lhs = (
                self.variables["carbon_emissions_technology_total"].loc[year] - term_summed_carbon_emissions_technology
            )
            rhs = 0
            constraints.append(lhs == rhs)
        ...
        ### return
        return self.constraints.return_contraints(
            constraints, model=self.model, index_values=years, index_names=["set_time_steps_yearly"]
        )
    ...
    def constraint_capacity_factor_block(self):
        ...
        for tech in index.get_unique(["set_technologies"]):
            ...
            term_capacity = (
                self.parameters.max_load.loc[tech, capacity_types, locs, times]
            ...
            if tech in self.sets["set_conversion_technologies"]:
                if reference_carrier in self.sets["set_input_carriers"][tech]:
                    ...
                    term_flow = (
                        -1.0 * self.variables["flow_conversion_output"].loc[tech, reference_carrier, locs, times]
                    )
            ...
            # storage technology
                ...
                # where true
                term_flow = mask * (
                    -1.0 * self.variables["flow_storage_charge"].loc[tech, locs, times]
                    - 1.0 * self.variables["flow_storage_discharge"].loc[tech, locs, times]
                )
