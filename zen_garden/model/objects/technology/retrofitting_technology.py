...
class RetrofittingTechnology(ConversionTechnology):
    ...
    def store_input_data(self):
        ...
        self.retrofit_flow_coupling_factor = self.data_input.extract_input_data(
            "retrofit_flow_coupling_factor", index_sets=["set_nodes", "set_time_steps"], unit_category={}
        )
...
class RetrofittingTechnologyRules(GenericRule):
    ...
    def constraint_retrofit_flow_coupling(self):
        ...
        rc_in = pd.Series(
            {
                ...
                for t, c in itertools.product(
                    self.sets["set_conversion_technologies"], self.sets["set_input_carriers"].superset
                )
            }
        )
        rc_out = pd.Series(
            {
                (t, c): True if c in self.sets["set_reference_carriers"][t] else False
                for t, c in itertools.product(
                    self.sets["set_conversion_technologies"], self.sets["set_output_carriers"].superset
                )
            }
        )
        ...
        for base, techs in self.system["set_exclusive_retrofitting_technologies"].items():
            ...
            assert (
                len(retrofit_base_technologies[retrofit_base_technologies.index.isin(techs)].unique()) == 1
                and mask == base
            ), "No retrofit base technology found in exclusive_installation"
            term_flow_reference_retrofit = flow_conversion_input.where(rc_in).loc[techs].sum(
                "set_input_carriers"
            ) + flow_conversion_output.where(rc_out).loc[techs].sum("set_output_carriers")
            lhs = (term_flow_reference_retrofit / retrofit_flow_coupling.loc[techs]).sum(
