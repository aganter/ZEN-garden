...
class BendersDecomposition:
    ...
    def generate_feasibility_cut(self, gurobi_model, subproblem) -> tuple:
        """
        ...
        merged_rhs_df = (
            pd.merge(farkas_multipliers_df, subproblem.rhs_cuts, on="labels_con")
            .assign(product=lambda df: df["multiplier"] * df["rhs"])["product"]
            .sum()
        )
        merged_lhs_df = pd.merge(farkas_multipliers_df, subproblem.lhs_cuts, on="labels_con").assign(
            product=lambda df: df["multiplier"] * df["coeffs"]
        )

        linear_expression_list = list(
            merged_lhs_df.loc[merged_lhs_df["product"] != 0, "product"] * merged_lhs_df["master_variable"]
        )

        feasibility_cut_rhs = merged_rhs_df
    ...
    def generate_optimality_cut(self, gurobi_model, subproblem) -> tuple:
        ...
        merged_rhs_df = (
            pd.merge(duals_multipliers_df, subproblem.rhs_cuts, on="labels_con")
            .assign(product=lambda df: df["multiplier"] * df["rhs"])["product"]
            .sum()
        )

        merged_lhs_df = pd.merge(duals_multipliers_df, subproblem.lhs_cuts, on="labels_con").assign(
            product=lambda df: df["multiplier"] * df["coeffs"]
        )
        linear_expression_list = list(
            merged_lhs_df.loc[merged_lhs_df["product"] != 0, "product"] * merged_lhs_df["master_variable"]
        ...
        optimality_cut_rhs = merged_rhs_df
