...
class BendersDecomposition:
    ...
    def new_list(self, gurobi_model, subproblem):
        """
        Generate the feasibility cut. The feasibility cut is generated by the Farkas multipliers of the subproblem
        model. In order to keep stability and speed up the process, only multipliers higher than 1e-4 are considered.
    ...
    def compare_linear_expressions(self, lhs, new_lhs, rhs, new_rhs):
        """
        Helper function to compare two linear expressions.
        """
        tolerance = 1e-10
        lhs_coeffs = lhs.coeffs.values
        new_lhs_coeffs = new_lhs.coeffs.values

        lhs_vars = lhs.vars
        new_lhs_vars = new_lhs.vars

        if lhs_coeffs.shape != new_lhs_coeffs.shape:
            logging.info("Shapes of coeffs are different.")
        else:
            coeffs_differences = np.where(np.abs(lhs_coeffs - new_lhs_coeffs) > tolerance)
            if len(coeffs_differences[0]) != 0:
                logging.info("")
            else:
                logging.info("Significant differences found in coeffs at indices:")
                for index in coeffs_differences[0]:
                    logging.info(
                        "Index %s: lhs_coeffs = %s, new_lhs_coeffs = %s",
                        index,
                        lhs_coeffs[index],
                        new_lhs_coeffs[index],
                    )

        if lhs_vars.shape != new_lhs_vars.shape:
            logging.info("Shapes of vars are different.")
        else:

            vars_differences = np.where(lhs_vars != new_lhs_vars)
            if len(vars_differences[0]) == 0:
                logging.info("")
            else:
                logging.info("Differences found in vars at indices:")
                for index in vars_differences[0]:
                    logging.info(
                        "Index %s: lhs_vars = %s, new_lhs_vars = %s", index, lhs_vars[index], new_lhs_vars[index]
                    )
        if not np.isclose(rhs, new_rhs, atol=tolerance):
            logging.info("RHS mismatch: %s != %s", rhs, new_rhs)
        else:
            logging.info("")
    ...
    def define_list_of_feasibility_cuts(self) -> list:
        ...
        new_feasibility_cuts = [
            (
                subproblem.scenario_name if subproblem.scenario_name else "default",
                *self.new_list(self.subproblem_to_gurobi(subproblem), subproblem),
            )
            for subproblem in infeasible_subproblems
        ]
        ...
        for i, (name, lhs, rhs) in enumerate(feasibility_cuts):
            new_name, new_lhs, new_rhs = new_feasibility_cuts[i]

            # Check if the names match
            if name != new_name:
                print(f"Name mismatch at index {i}: {name} != {new_name}")

            self.compare_linear_expressions(lhs, new_lhs, rhs, new_rhs)

        return feasibility_cuts, new_feasibility_cuts
    ...
    def fit(self):
        ...
        while continue_iterations and iteration <= max_number_of_iterations:
            ...
            if any(subproblem.model.termination_condition != "optimal" for subproblem in self.subproblem_models):
                ...
                feasibility_cuts, _ = self.define_list_of_feasibility_cuts()
