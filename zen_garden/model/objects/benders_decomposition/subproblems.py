...
class Subproblem(OptimizationSetup):
    ...
    def set_objective_to_constant(self):
        """
        Set the objective function of the subproblem to a constant value.
        """
        self.variables.add_variable(
            self.model,
            name="mock_objective_subproblem",
    ...
    def get_linopy_variable(self, row) -> object:
        ...
        return self.master_model.model.variables[row["labels_var"]].sel(row["pos_var"])
    ...
    def define_lhs_rhs_for_cuts(self):
        ...
        """
        folder_path = os.path.join(self.config.analysis.dataset, self.config.benders.input_path, "rhs_lhs_cuts")
        ...
        labels_pos = self.master_model.model.variables.get_label_position(self.lhs_cuts["labels"])
        labels, pos = zip(*labels_pos)
        self.lhs_cuts["labels_var"] = labels
        self.lhs_cuts["pos_var"] = pos
        # Adding also the corresponding master variable of the Linopy model
        # label_position_dict = {i: self.master_model.model.variables[label_position[i][0]].sel(label_position[i][1]) for i in idx}
        # self.master_model.model.variables[f"{label_position[0]}"].sel(label_position[1])
