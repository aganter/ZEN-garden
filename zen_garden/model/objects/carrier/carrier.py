...
class CarrierRules(GenericRule):
    ...
    def constraint_import_share_block(self):
        ...
        mask = (self.parameters.availability_import != np.inf).where(mask, False)

        ### index loop
        # not necessary

        ### formulate constraint
        lhs = self.variables["flow_import"].where(mask)
        rhs = (self.parameters.availability_import * self.parameters.import_share).where(mask)
        constraints = lhs <= rhs
