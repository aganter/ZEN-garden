...
class OptimizationSetup(object):
    ...
    def overwrite_params(self, scenario, elements):
        ...
        for element_name, params in elements.items():
            ...
            # overwrite scenario dependent parameters
            for param, scen in params.items():
                scen = "_" + scen
                ...
                if isinstance(_old_param, float):
                    _index_names = []
                    _new_param = element.data_input.extract_input_data(file_name, index_sets = [], scenario=scen)[0]
                ...
                _time_steps = None
                # if existing capacity is changed, set_technologies_existing, existing lifetime, and capex_existing_capacity have to be updated as well
                if "set_technologies_existing" in _index_sets:
                    # update set_technologies_existing and lifetime_existing
                    _technologies_existing = element.data_input.extract_set_technologies_existing(scenario=scen)
                    setattr(element, "set_technologies_existing", _technologies_existing)
                    _lifetime_existing = element.data_input.extract_lifetime_existing(param, index_sets=_index_sets, scenario=scen)
                ...
                if hasattr(element, "raw_time_series") and param in element.raw_time_series.keys():
                    ...
                    element.raw_time_series[param] = element.data_input.extract_input_data(file_name, index_sets=_index_sets, time_steps=_time_steps, scenario=scen)
                else:
                    ...
                    _new_param = element.data_input.extract_input_data(file_name, index_sets=_index_sets, time_steps=_time_steps, scenario=scen)
                    setattr(element, param, _new_param)
    ...
    def solve(self, solver):
        ...
        if solver_name == "gurobi_persistent":
            ...
            self.results = self.opt.solve(tee=solver["verbosity"], options_string=solver_parameters,save_results=False, load_solutions=False)
            # logfile=solver["solver_options"]["logfile"]
            self.opt.load_vars()
        elif solver_name == "gurobi":
            self.opt = pe.SolverFactory(solver_name, options=solver_options)
            self.results = self.opt.solve(self.model, tee=solver["verbosity"], keepfiles=True, symbolic_solver_labels=solver["use_symbolic_labels"], options_string=solver_parameters)
            #logfile = solver["solver_options"]["logfile"],
        else:
    ...
    def add_new_capacity_addition(self, step_horizon):
        ...
        if self.system["use_rolling_horizon"]:
            if step_horizon != self.energy_system.set_time_steps_yearly_entire_horizon[-1]:
                capacity_addition = pd.Series(self.model.capacity_addition.extract_values())
                invest_capacity = pd.Series(self.model.capacity_investment.extract_values())
                cost_capex = pd.Series(self.model.cost_capex.extract_values())
