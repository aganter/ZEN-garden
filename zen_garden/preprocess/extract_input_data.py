...
class DataInput:
    ...
    def extract_input_data(self, file_name, index_sets, unit_category, time_steps=None, subelement=None):
        ...
        if (file_name == "capacity_existing" or file_name == "capacity_existing_energy") and not self.system[
    ...
    def extract_general_input_data(self, df_input, df_output, file_name, index_name_list, default_value, time_steps):
        ...
        assert default_value is not None or len(common_index) == len(
            df_output.index
        ), f"Input for {file_name} does not provide entire dataset and no default given in attributes.csv"
    ...
    def _load_attribute_file_json(self, filename):
        ...
        if type(data) == list:
            logging.warning(
                "DeprecationWarning: The list format in attributes.json [{...}] is deprecated. Use a dict format instead {...}."
            )
            for item in data:
    ...
    def extract_attribute(self, attribute_name, unit_category, return_unit=False, subelement=None):
        ...
        if attribute_value is not None:
            multiplier, attribute_unit_in_base_units = self.unit_handling.convert_unit_into_base_units(
                attribute_unit,
                get_multiplier=True,
        ...
        if extract_nodes:
            ...
            if extract_coordinates:
                ...
                if len(set_nodes_config) == 0 and not len(set_nodes_input) == 0:
                    ...
                    ), f"ZENx is a spatially distributed model. Please specify at least 2 nodes."
                    _missing_nodes = list(set(set_nodes_config).difference(set_nodes_input))
                    assert (
                        len(_missing_nodes) == 0
                    ), f"The nodes {_missing_nodes} were declared in the config but do not exist in the input "
                    f"file {os.path.join(self.folder_path, 'set_nodes')}"
                ...
                if value_variable == breakpoint_variable:
                    ...
                    if (
                        ...
                        values_between_bounds = [
                            ...
                            for idx_breakpoint, breakpoint in enumerate(breakpoints)
            ...
            if string_row.any():
                ...
                if isinstance(unit_row, str):
                    ...
                    multiplier = unit_row.apply(
                        lambda unit: self.unit_handling.get_unit_multiplier(unit, attribute_name="capex")
                    )
    ...
    def save_unit_of_attribute(self, attribute_name, subelement=None):
        ...
        if self.solver["analyze_numerics"] and attribute_name is not None:
            input_unit = self.extract_attribute(
                attribute_name,
                unit_category=None,
                subelement=subelement,
                return_unit=True,
            ...
            self.unit_handling.set_base_unit_combination(
                input_unit=input_unit, attribute=(self.element.name, attribute_name)
            )
        ...
        if self.solver["analyze_numerics"]:
            if file_name:
                ...
                if not df_output_reduced.empty:
                    self.unit_handling.set_attribute_values(
                        df_output=df_output_reduced,
                        attribute=(self.element.name, file_name),
                    )
